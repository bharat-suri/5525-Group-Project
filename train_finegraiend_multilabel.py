...
from dataset import Persuation, create_loader
...
if __name__ == '__main__':
    ...
    train_tsv = "data/propoganda/training.json"
    dev_tsv =  "data/propoganda/dev.json"
    test_tsv = "data/propoganda/dev.json"

    train_tsv = "data/data_task6/training.json"
    dev_tsv =  "data/data_task6/dev.json"
    test_tsv = "data/data_task6/test.json"
    loss_type = "BCE"

    args.train_path = train_tsv
    # Setup datasets.
    train_ds = Persuation(args, train_tsv)
    valid_ds = Persuation(args, dev_tsv)
    ...
    log_file = "logs/%s_%s_multilabel_trainedOn_%s_epoch%s.csv"%(args.model, args.bias_type, 'ff', args.epochs)
    ...
    with open(log_file, "w") as logfile:
        ...
        logwriter.writerow(['epoch', 'train loss', 'valid loss', 'valid acc',  'valid_f1_score_micro',  'valid_f1_score_macro', 'training time'])
    for epoch in range(1, args.epochs+1):
        start_time = time.time()
        train_loss = train(args, loss_type)
        valid_loss, valid_accuracy, valid_f1_score_micro,  valid_f1_score_macro = valid(args)
        end_time = time.time()
        ...
        with open(log_file, 'a') as logfile:
            ...
            logwriter.writerow([epoch, train_loss, valid_loss, valid_accuracy, str(valid_f1_score_micro),  str(valid_f1_score_macro), end_time-start_time])
        print(
            f'epoch: {epoch} | '
        ...
        if valid_loss < best_loss or valid_f1 > best_f1:
            ...
            args.ckpt = "ckpt/model_%s_trainedOn_%s_seed%s_multilabel_%s.pt"%(args.model, "raw" if "propoganda" not in train_tsv else "propoganda", args.rand_seed, loss_type)
            if args.pretrained_ckpt != "":
    ...
    # logger.log('test', str(accuracy), str(f1_score_micro),  str(f1_score_macro), str(prf1report))
