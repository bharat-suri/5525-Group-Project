...
def _read_gold_and_pred(pred_fpath, gold_fpath):
  ...
  with open(gold_fpath, encoding='utf-8') as gold_f:
    ...
    for obj in gold:
      _labels = []
      for small_label in obj['labels']:
        if small_label.startswith("Thought-terminating"):
          small_label = "Thought-terminating"
        _labels.append(small_label)
      gold_labels[obj['id']] = _labels
