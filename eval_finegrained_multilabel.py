...
from dataset import BiasDataset, BiasDataset_multilabel, create_loader, Persuation
...
def test(args, name):
    ...
    for i in range(len(ids)):
        labels_name_ = [get_key(index, test_ds.name2id) for index in np.where(y_pred_value[i] != 0)[0].tolist()]
...
def write_out(args, write_file):
    ...
    for i, (inputs, labels) in enumerate(test_loader):
        ...
        y_pred.extend(prediction)
        for i in range(inputs.size()[0]):
            write_file.write(str(cur_label[i]) + " |||" + str(prediction_value[i]) + "\n")
    # print(, np.array(y_pred.shape))
    # roc_auc = cal_roc_auc(np.array(y_true), np.array(y_pred))
    # targets, outputs = y_true, np.array(y_pred) >= 0.5
    # accuracy = metrics.accuracy_score(targets, outputs)
...
if __name__ == '__main__':
    ...
    for name in [ "model_bert_trainedOn_raw_seed487_multilabel_BCE.ptfurther_tune"]:
    #[ "model_bert_trainedOn_raw_seed857_multilabel_BCE.ptfurther_tune", "model_bert_trainedOn_raw_seed777_multilabel_BCE.ptfurther_tune", "model_bert_trainedOn_raw_seed251_multilabel_BCE.ptfurther_tune","model_bert_trainedOn_raw_seed457_multilabel_BCE.ptfurther_tune","model_bert_trainedOn_raw_seed467_multilabel_BCE.ptfurther_tune"]:
    #name = "model_bert_trainedOn_raw_seed857_multilabel_BCE.ptfurther_tune"
        print(name)
        test_tsv = "data/data_task6/test.json"
        logger = Logger(args.name)

        # Setup datasets.
        test_ds = Persuation(args, test_tsv)
        # Setup vocab.
        if args.model == "bert":
            args.n_emb = 30522
        ...
        ckpt_path = 'ckpt/%s'%name
        model.load_state_dict(torch.load(ckpt_path, map_location="cpu"))
        print("Test on ...", test_tsv)
        accuracy, f1_score_micro,  f1_score_macro, prf1, classification_report, roc_auc = test(args, name)
        all_accuracy.append(accuracy)
        all_micro_f1.append(f1_score_micro)
        all_macro_f1.append( f1_score_macro)
        all_auc.append(roc_auc)
        import numpy as np 
        import os 
        os.system("python scorer/task1_3.py --gold_file_path=data/data_task6/test.json --pred_file_path=output/%s.json --classes_file_path=techniques_list_task1-2.txt > eval_scores/%s"%(name, name))
